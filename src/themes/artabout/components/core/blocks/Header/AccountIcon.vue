<template>
  <div
    class="inline-flex relative dropdown no-outline"
  ...
  >
    ...
    >
      <i class="cl-white material-icons block">account_circle</i>
    </button>

    <no-ssr>
      <div v-show="currentUser" :class="['bg-cl-white dropdown-content align-left sans-serif lh20 weight-400', !showMenu ? 'dropdown-content__hidden' : '']">
        <div class="py5">
          <div v-for="(page, index) in navigation" :key="index" @click="notify(page.title)">
            <router-link @click.native="showMenu = false" class="cl-black no-underline block py10 px15" :to="localizedRoute(page.link)">
              {{ page.title }}
            </router-link>
          </div>
        </div>
