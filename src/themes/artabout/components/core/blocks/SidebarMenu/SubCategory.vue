<template>
  <div>
    <ul v-if="children" class="sidebar-submenu absolute w-100 p0 bg-cl-primary" :style="styles">
      <li v-if="parentSlug" class="brdr-bottom-1 brdr-cl-bg-secondary bg-cl-black flex">
        <router-link
    ...
    >
      <li
        class="brdr-bottom-1 brdr-cl-bg-secondary bg-cl-black flex cl-white"
        :key="link.id"
        v-for="link in myAccountLinks"
        @click="notify(link.name)"
      >
        <router-link
          class="px25 py20 cl-white no-underline col-xs"
          :to="localizedRoute(link.url)"
        >
          {{ link.name }}
        </router-link>
      </li>
      <li class="brdr-bottom-1 brdr-cl-bg-secondary bg-cl-black flex cl-white">
        <a
          href="#"
          class="px25 py20 cl-white no-underline col-xs"
